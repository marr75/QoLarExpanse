...
static class CargoStacking {
    ...
    internal static bool IsMultiStackMember(Cargo? cargo, CargoAll? owner) {
        if (!IsStackable(cargo, owner)) { return false; }
    ...
    internal static StackPlan Build(ResourcesList list) {
        ...
        foreach (var row in rows) {
            ...
            if (cargo != null && !rowOf.ContainsKey(cargo)) { rowOf[cargo] = row; }
        }

        var stacks = new List<StackEntry>();
        var hidden = new List<ResorceRow>();
        if (owner == null) { return new StackPlan(rows, stacks, hidden); }
    ...
    static void Collect(
        CargoGroup group,
        IReadOnlyDictionary<Cargo, ResorceRow> rowOf,
        List<StackEntry> stacks,
        List<ResorceRow> hidden,
        bool editable
    ) {
        var placed = new List<(Cargo cargo, ResorceRow row)>();
        foreach (var member in group.Members) {
            if (rowOf.TryGetValue(member, out var row)) { placed.Add((member, row)); }
        }
        ...
        stacks.Add(
            new StackEntry(placed[first].row, new CargoGroup(group.ResourceTypeType, group.Key, members), editable)
        );
    }
    ...
    static IReadOnlyList<ResorceRow> DistinctRows(ResourcesList list) {
        ...
        foreach (var row in list.listResorces) {
            if (row && seen.Add(row)) { rows.Add(row); }
        }
        return rows;
    }
}
...
sealed class StackPlan {
    internal StackPlan(
        IReadOnlyList<ResorceRow> rows,
        IReadOnlyList<StackEntry> stacks,
        IReadOnlyList<ResorceRow> hidden
    ) {
        Rows = rows;
        Stacks = stacks;
        Hidden = hidden;
