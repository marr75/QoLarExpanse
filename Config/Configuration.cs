...
sealed class Configuration {
    ...
    public Configuration(ConfigFile c) {
        const string masterEnableDescription =
            "Master switch for every quality-of-life feature in this mod. Off = the game behaves exactly like "
        ...
        const string bodyNavEnableDescription =
            "Step between planets (and their moons) with hotkeys instead of hunting for them on the map. "
        ...
        const string orbitalClickEnableDescription =
            "Ctrl-click a body's surface to jump straight to its orbital view (and back). Skips the extra "
        ...
        const string orbitalDragTargetingEnableDescription =
            "Ctrl-drag a resource, module, or spacecraft onto a body to default the drop to that body's "
        ...
        const string toggleViewEnableDescription =
            "Flip the currently open info window between a body's surface and its orbit with one key.";
        ...
        const string screenHotkeysEnableDescription =
            "Open the main screens (search, missions, market, research) directly with hotkeys. Turns on the "
            + "screen keys below.";
        ...
        const string arrowPanGuardEnableDescription =
            "Stop arrow keys from panning the camera while a screen or text field is open, so screen hotkeys "
        ...
        const string suppressOnLoadContractEnableDescription =
            "Stop the contract-info window (and its announce sound) from auto-opening for each restored "
            + "contract while a save loads. Normal in-game contract popups are unaffected.";
        SuppressOnLoadContractEnabled = c.Bind(
            "Save",
            "SuppressOnLoadContractEnabled",
            true,
            suppressOnLoadContractEnableDescription
        );

        const string searchNavEnableDescription =
            "Move through a search field's suggestion list with Up/Down and commit the highlighted one with "
        ...
        const string dropAllEnableDescription =
            "Adds \"drop all\" / \"drop all of type\" buttons to the mission cargo panel so you can clear cargo "
        ...
        const string reorderEnableDescription =
            "Reorder cargo and module rows in the mission panel. Reserved for a future update - off and unwired "
            + "for now.";
        ...
        const string unifiedCrewSliderEnableDescription =
            "Replace the per-module crew sliders with one unified crew/population slider that distributes "
            + "across every crew-capable module.";
        UnifiedCrewSliderEnabled = c.Bind(
            "Cargo",
            "UnifiedCrewSliderEnabled",
            true,
            unifiedCrewSliderEnableDescription
        );

        const string crewAllocationPolicyDescription =
            "How the unified crew slider spreads crew across modules. Proportional (default) splits by each "
            + "module's share of capacity; FillFirst fills modules one at a time in order.";
        CrewAllocationPolicy = c.Bind(
            "Cargo",
            "CrewAllocationPolicy",
            Config.CrewAllocationPolicy.Proportional,
            crewAllocationPolicyDescription
        );

        const string deferOrbitalClickToUXTweaksDescription =
            "Compatibility escape hatch: disable this mod's Ctrl-click orbital handling so UXTweaks' own "
